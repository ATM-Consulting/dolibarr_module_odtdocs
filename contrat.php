...
//	print_r($tableau); exit;
	...
	TODTDocs::makeDocTBS(
		'contract'
		, $_REQUEST['modele']
		,array('mysoc'=>$mysoc, 'societe'=>$societe,'conf'=>$conf, 'contrat'=>$contrat )
		,$fOut
		, $conf->entity
...
function _get_infos_propal($idContrat){
	
	$sql= 'SELECT pr.id as prop_id, pr.ref as prop_ref FROM '.MAIN_DB_PREFIX.'contrat c
	INNER JOIN '.MAIN_DB_PREFIX.'projet proj ON (c.fk_projet=proj.rowid)';
