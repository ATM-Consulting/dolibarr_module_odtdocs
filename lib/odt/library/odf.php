...
class Odf
{
    ...
    public function setVars($key, $value, $encode = true, $charset = 'ISO-8859')
    {
        if (strpos($this->contentXml, $this->config['DELIMITER_LEFT'] . $key . $this->config['DELIMITER_RIGHT']) === false) {
           return false; // throw new OdfException("var $key not found in the document");
        }
     ...
     * @return odf
     */
    public function setImage($key, $value, $widthref=-1)
    {
        ...
        if ($size === false) {
           ...
           print "Invalid image ". $value .'<br>';
           return false;
        }
		list ($width, $height) = $size;
		
        if($width<0) { $coef = 1; }
		else {	$coef = $widthref / $width;	}
		$width = round($width * $coef);
		$height = round($height * $coef);
		
        $width *= self::PIXEL_TO_CM;
        $height *= self::PIXEL_TO_CM;
        $xml = '<draw:frame draw:style-name="fr1" draw:name="'.$filename.'" text:anchor-type="char" svg:x="0cm" svg:y="0cm" svg:width="'.$width.'cm" svg:height="'.$height.'cm" draw:z-index="0"><draw:image xlink:href="Pictures/'.$file.'" xlink:type="simple" xlink:show="embed" xlink:actuate="onLoad"/></draw:frame>';
